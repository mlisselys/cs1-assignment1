...
// Student Name: Lisselys Martinez
// File Name: pa1_martinez.c
...
typedef struct {
    ...
    int card_count; //number of cards the player has
    card_t* list_of_cards; //dynamic list of card_t. Dynamically allocated based on card_count
}player_t;
...
player_t *create_player_list(int list_count) {
    ...
    player_t *player_list = (player_t*)malloc(list_count * sizeof(player_t));
...
void initialize_player(player_t *player, char *name, int card_count) {
    ...
    if(player->name == NULL)
    {
        printf("Memory allocation failed for player name\n");
        return;
    }

    strcpy(player->name, name); //copy the name to the allocated memory

    //setting the card count
    player-> card_count = card_count;

    player-> list_of_cards = (card_t *)malloc(card_count * sizeof(card_t));
    if(player-> list_of_cards == NULL)
    {
        printf("Memory allocation failed for list of cards\n");
        free(player->name);
        return;
    }
...
void display_player_information(player_t *player) {
    ...
    */
    ...
    for(int i = 0; i < player->card_count; i++)
    {
        ...
        total_rank += player->list_of_cards[i].rank;
    }
